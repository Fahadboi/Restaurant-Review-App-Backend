...
export const verifyRestaurantJWT = asyncHandler(async (req, res, next) => {
    try {
        ...
        if (!token) {
            throw new ErrorHandler(401, "Cannot verify the token - Please login again");
        }

        const user = await Restaurant.findById(decodedToken._id).select("-password -refreshToken");
